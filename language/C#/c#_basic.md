...
- `컴파일 타임`에서 `타입 안정적인 코드`가 작성 가능
- 특히 컬렉션, 델리게이트, 비동기 코드, Task, null 가능 값 타입에 많이 사용
- enum은 제네릭이 될 수 없고, 클래스, 구조체, 인터페이스, 델리게이트는 모두 제네릭이 될 수 있다.
- 메서드의 인수는 자동으로 `타입 추론`이 가능하다. -> 코드를 짧게 만들어 준다.

<br>

> `.NET 1`의 `배열`과 `특화된 컬렉션`은 한 가지의 고정타입밖에 넣어줄 수 없었다.

1. `Array`는 배열의 크기 재할당 작업이 어려웠다.
2. `ArrayList`는 다른 타입의 객체를 저장할 위험이 있었다.
3. `특화 컬렉션`인 StringCollection은 하나의 타입만 저장할 수 있었다.
4. `자체 컬렉션`을 개발하려면 개발자가 계속 시간을 써야했다.

=> 이에따라 `C# 2`에서 `Generic`이 등장하게 되었다.
...
> 여기서 T, U는 `타입 매개변수`, T, U 대신 들어가는 타입은 `타입 인수`라고 부른다.
>
> T, U와 같이 사용된 타입 매개변수의 개수를 `애리티`라고 부른다. 같은 이름의 제네릭이여도 애리티가 다르면 서로 다른 메서드로 구분된다.
...
여기서 T는 IFormattable 인터페이스를 구현한 것만 가능

1. `참조 타입 제약조건(where T : class)` : 타입 인수는 참조 타입(인터페이스, 델리게이트, 클래스 등 참조타입)만 가능
2. `값 타입 제약 조건(where T : struct)` : null 가능 타입이 아닌 값 타입
3. `생성자 조건(where T : new())` : 타입 인수는 매개변수가 없는 생성자를 가져야 함
4. `변환 조건(where T : SomeType)` : SomeType은 클래스, 인터페이스 혹인 다른 타입 매개변수가 될 수 있음

<br>

### default

피연산자로 타입의 이름이나 매개변수를 취하여 해당 타입의 `기본값`을 돌려줌
